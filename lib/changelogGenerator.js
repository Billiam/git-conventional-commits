module.exports = function (config) {
     ...
     * @param {string} generatorParams.fromTag - the previous release tag.
     * @param {string} generatorParams.releaseName - the version number of the release including an optional suffix.
     * @param {string} [generatorParams.customMarkdownPrefix] - optional custom markdown to be prefixed in the changelog.
    ...
    this.generateMarkdown = function (
        ...
        // ------ generate custom prefix section
        if (customMarkdownPrefix && customMarkdownPrefix.length) {
            changelogMarkdown += generateCustomMarkdownSection(customMarkdownPrefix);
        }

        // ------ generate version headline
        let changelogCommitUrl = markdownCommitUrl(fromTag, releaseName);

        changelogMarkdown +=
            `## [${releaseName || lastChangelogCommit.hash}](${changelogCommitUrl}) ` +
            `(${yyyy_mm_dd(lastChangelogCommit.date)})\n\n`;
    ...
    function markdownIssueId(issueId) {
        if (config.issueUrl) {
            ...
            let name = issueId
            if (config.issueName) {
                name = config.issueName.replace("%id%", name)
            }
            return markdownLink(name, issueUrl);
        }
    ...
    function markdownCommitUrl(fromCommitHash, toCommitHash) {
        return config.commitRangeUrl.replace("%from%", fromCommitHash).replace("%to%", toCommitHash);
    }

    function markdownCommitRange(formCommitHash, toCommitHash) {
        if (config.commitRangeUrl) {
            const commitRangeUrl = markdownCommitUrl(formCommitHash, toCommitHash)
